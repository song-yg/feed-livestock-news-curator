...
#
# 2026-07-24 갱신: "축산물 수입관세" -> "축산물 할당관세". 실제 검색해보니
# "축산물 수입관세"라는 정확한 문구를 그대로 쓰는 기사가 실질적으로 없고,
# 실제 기사들은 전부 "할당관세"라는 용어를 씀(예: "수입품 할당관세 상시화
# 불가피", "ASF 살처분 돼지 12만 두 이상...정부 '할당관세 검토'") - 네이버
# 뉴스 검색이 형태소 분석 없이 비교적 문자열 그대로 매칭하는 경향이 있어,
# 실제 기사에 없는 조합을 검색하면 0건이 나오는 게 자연스러운 결과였음
# (담당자가 이번 주 실행에서 이 키워드만 계속 0건인 걸 확인하고 지적).
# "할당관세" 단독으로 하면 축산물 아닌 일반 농산물(양파·마늘 등) 기사까지
# 섞여 들어올 위험이 있어, "축산물" 앵커는 그대로 유지.
#
# 2026-07-23 갱신: 시트 v5(카테고리당 1개 원칙 + 앵커 복합어, 알고리즘 문서
# "키워드 시트 v5" 참고)의 현재 활성 ko 키워드와 동기화함. 예전 값
# (["조류독감", "구제역", "사료 가격", "축산물 수급"])엔 이미 위험하다고
# 확인된 "구제역"(유튜버 닉네임과 동음이의 - 협박 소송 기사가 국내/해외
# Top5를 도배한 사고 실측 확인, 2026-07-23)이 그대로 남아있었음 - 시트
# 타임아웃(2026-07-23, 30초로 완화됨) 등으로 fallback이 발동할 때마다
# 이 위험이 재발할 수 있어 동기화함. 시트가 나중에 또 바뀌면 이 fallback도
# 수동으로 같이 갱신해줘야 함(자동 동기화 아님 - keyword_source.py는 시트
# 실패 시 이 하드코딩 값을 그대로 쓸 뿐, 시트 변경을 감지해 여기로 역으로
# 반영하는 기능은 없음).
